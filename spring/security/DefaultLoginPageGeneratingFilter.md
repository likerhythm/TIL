...
이 필터는 로그인 페이지를 만드는 역할을 수행한다. 로그인 페이지를 html로 만들고 이를 response로 넘긴다.
여러 로그인 설정을 할 수도 있다. 예를 들어 로그인 성공 시 redirect할 URL, 사용자 이름(username)을 입력할 필드 이름 등이 있다.
여러 인증 방식을 활성화할 수도 있다. form 기반의 로그인, OAuth 2.0 기반의 로그인, SAML 2.0 기반의 로그인을 활성화 하거나 비활성화할 수 있다.

아래는 `DefaultLoginPageGeneratingFilter`의 전문인데, 코드 길이가 길긴 하다. 하지만 각 설정에 따른 HTML 코드를 생성하는 로직이 대부분이기 때문에 어렵진 않다.
인증 과정은 UsernamePasswordAuthenticationFilter에서 모두 수행했으므로 이 필터에서는 인증 관련 로직은 따로 진행하지 않는다.
```java
/*
